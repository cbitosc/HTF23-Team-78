...
  }
    ...
    useEffect(() => {
        const searchTerms = props.content.toLowerCase().split(',');
    
        const temp = data.filter((recipe) => {
          // Check if the dishName includes any of the search terms
          if (searchTerms.some((term) => recipe.dishName.toLowerCase().includes(term))) {
            return true;
          }
    
          // Check if any ingredient includes any of the search terms
          if (
        ...
        setArr(temp);
      }, [props.content,searchbar]);


    return (
        <div className='m-[50px] gap-y-3 flex flex-col'>
            {
                arr.map((recipe, index) => (
                    <div>
                         <Link to='/RecipeDetails' className="no-underline"><div key={index} className="p-2 border-1 border-grey shadow-md flex w-[800px] h-[150px] gap-2 rounded-lg cursor-pointer"
                                onClick={() => { props.getcontentfromsearch(recipe.image) }}>

                                <div className='flex items-center  justify-center w-[300px]'>
                                    <img src={`${recipe.image}`} className='w-full h-full object-cover' alt={recipe.dishName} />
                                </div>
                                <div className='m-[10px] w-[500px]'>
                                    <h5 className='text-black text-center'>{recipe.dishName}</h5>
                                    <p className='text-[grey]'>{recipe.description}</p>
                                </div>
                            </div>
                            </Link>
