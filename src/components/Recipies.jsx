...
  }
    ...
    const prevSlide = (diffcusine) => {
        setCurrentIndex((prevIndex) =>
            prevIndex === 0 ? diffcusine.length - 5 : prevIndex - 1
        );
        diffcusine[diffcusine.length - 1] = (diffcusine[diffcusine.length - 1] === 0 ? 0 : diffcusine[diffcusine.length - 1] - 1)
    };

    const nextSlide = (diffcusine) => {
        setCurrentIndex((prevIndex) =>
            prevIndex === diffcusine.length - 5 ? 0 : prevIndex + 1
        );
        diffcusine[diffcusine.length - 1] = (diffcusine[diffcusine.length - 1] === diffcusine.length - 5 ? diffcusine.length - 5 : diffcusine[diffcusine.length - 1] + 1)
    };

    const [categorizedArray, setcategorizedArray] = useState([]);
    useEffect(() => {
        // Create an object to store categorized arrays
        let categorizedData = data.reduce((result, item) => {
            // Get the cuisine type from the current item
            let cuisine = item.cuisine;

            // Check if the cuisine key exists in the result object, if not, create an empty array
            if (!result[cuisine]) {
                result[cuisine] = [];
            }

            // Push the current item into the corresponding cuisine array
            result[cuisine].push(item);

            return result;
        }, {});

        // Convert the categorizedData object into an array of arrays
        Object.values(categorizedData).forEach((ar) => ar.push(0));
        console.log(Object.values(categorizedData))
        setcategorizedArray(Object.values(categorizedData))
    }, [data.length])
    ...
    return (
        <div >
            <div>
                {
                    categorizedArray.map((diffcusine, ind) => (
                        <div className='w-full '>
                            {
                                diffcusine.length > 0 ? <div className='p-3 text-center'>
                                    <h2>{diffcusine[0].cuisine}</h2>
                                    <div key={ind} className='flex'>
                                        <button onClick={() => prevSlide(diffcusine)}>
                                            <BsChevronLeft />
                                        </button>
                                        <div className='flex gap-2' style={{ backgroundSize: "cover" }} >
                                            {diffcusine.slice(diffcusine[diffcusine.length - 1], diffcusine[diffcusine.length - 1] + 4).map((item, index) => (
                                                <Link to="/RecipeDetails" className="no-underline"><div onClick={() => props.getcontentfromrecipe(item.image)}>
                                                    {(index != diffcusine.length - 1) && <div key={index} className="text-white  p-2 border-1 border-grey shadow-md flex flex-col w-[300px] h-[300px] gap-y-2">

                                                        <div className='flex items-center  justify-center  h-[150px]' >
                                                            <img src={`${item.image}`} className='w-full h-full object-cover' alt={item.dishName} />
                                                        </div>
                                                        <div className='h-[150px] flex flex-col flex-grow' >
                                                            <h5 className='text-black text-center  '>{item.dishName}</h5>
                                                            <p className='text-[grey]  object-cover w-[250px] overflow-hidden text-ellipsis inline'>{item.description}</p>
                                                        </div>
                                                    </div>}
                                                </div>
                                                </Link>
                                        ...
                                        <button onClick={() => nextSlide(diffcusine)}>
                                            <BsChevronRight />
                                        </button>
                                    </div>
                                </div> : null}
