...
  }
    ...
    useEffect(() => {
        const categorizedData = data.filter((item) => {
            ...
            return cuisine === props.selectedcuisine.toLowerCase();
        });
    ...
    return (
        <div>
          {console.log("cateforized is",categorizedArray)}
            <div className='m-[50px] gap-y-3 flex flex-col'>
                {
                    categorizedArray?.map((recipe, index) => (
                        <Link to="/RecipeDetails" className='no-underline'>
                        <div onClick={()=>props.getcontentfromcuisine(recipe.image)}>

                            <div key={index} className="p-2 border-1 border-grey shadow-md flex w-[600px] h-[150px] gap-2 rounded-lg">
